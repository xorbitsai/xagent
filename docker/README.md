...
| `INSTALL_CHARTMOGUL` | `true` | Vendors a pinned commit of `chartmogul-mcp-server` (github.com/chartmogul/chartmogul-mcp-server) into the image for the built-in ChartMogul MCP connector, which — unlike every other stdio connector here — ships no PyPI/npm package at all, so this build clones its source directly and runs `uv sync` against it. Pass `--build-arg INSTALL_CHARTMOGUL=false` to skip the clone, the extra `uv`-managed Python 3.13 interpreter download (the vendored server requires >=3.13; the base image ships 3.11), and the image weight, for deployments that don't use it or whose build environment can't reach `github.com`. The connector ships hidden from the connector catalog regardless of this flag (Docker-only: the non-Docker `pip install xagent-ai` install path never runs this Dockerfile, so it never has anything at the vendored path). `CHARTMOGUL_MCP_REPO_URL`/`CHARTMOGUL_MCP_REF`/`CHARTMOGUL_MCP_VENDOR_PATH` are separate build args for the source URL, pinned commit, and destination path respectively — these exist mainly so the Dockerfile doesn't hardcode them twice, not as knobs deployments are expected to override. |
