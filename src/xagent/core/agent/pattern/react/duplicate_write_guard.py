...
* Same turn only, enforced by the ledger record's guard turn identity. An
  ordinary completion uses its original turn id. A resumed settlement uses
  the settlement turn that received the outcome, so a model cannot
  immediately repeat the approved — or denied — write after resume. A later
  user turn remains a new authorization boundary and may issue the same
  arguments again. A call with no guard turn id (an embedding that drives
  the pattern directly without stamping turns) is never guarded:
...
def build_suppression_envelope(
    *,
    ...
    A successful prior call produces a successful envelope: the requested
    effect exists — it was produced by the earlier call — so the loop's
    success accounting should treat this observation like the write it stands
    in for. A denied or dispatch-unknown settlement stays unsuccessful while
    still preventing the write from being executed again.
    """
