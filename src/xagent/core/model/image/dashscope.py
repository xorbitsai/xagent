...
class DashScopeImageModel(BaseImageModel):
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.post(
                    ...
                    # Inside the billing boundary: the 200 is already charged, so a
                    # body that will not decode is an invalid response, not a
                    # retryable failure. Left outside, a JSONDecodeError reached
                    # the retry predicate and the call was re-billed with no
                    # metering row ever written.
                    try:
                        response_data = await decode_billed_body_async(
                            response.json, "DashScope returned an undecodable body"
                        )
                    except (InvalidImageResponseError, asyncio.CancelledError):
                        # Metered before re-raising. DashScope charged for this
                        # 200; classifying it as non-retryable stops the repeat
                        # charges, but the one real charge still needs a row.
                        #
                        # CancelledError is named explicitly because it is a
                        # BaseException: the boundary's `except Exception` never
                        # sees it, so a task cancelled mid-decode would leave the
                        # charge with no row at all. Recorded here and re-raised
                        # unchanged -- swallowing a cancellation would break the
                        # caller's shutdown.
    ...
    async def edit_image(
        ...
        **kwargs: Any,
        ...
        # Fold the size-shaping parameters into `size` before kwargs is spread
        # into the request, applying the same precedence generate_image does.
        # DashScope takes only `size`, so the others are popped rather than
        # forwarded -- and the recorded resolution below reads the folded value,
        # so an edit asked for at 1920x1080 is billed at 1920x1080.
        folded_size = resolve_requested_size(
            kwargs.pop("size", None),
            resolution=kwargs.pop("resolution", None),
            width=kwargs.pop("width", None),
            height=kwargs.pop("height", None),
            # No invented default: an unset size means DashScope picks, and
            # recording a tier nobody asked for would bill against a key the
            # request never used.
            default="",
        )
        kwargs.pop("aspect_ratio", None)
        # Two separate things the request needs, and they are not the same
        # string. DashScope's wire form is `W*H`, which generate_image already
        # normalises; the recorded resolution stays `WxH` so one physical size
        # cannot file under two aggregate keys. And an absent size is an absent
        # key: sending `size=""` rejects a no-size edit the base request would
        # have accepted.
        # One physical size, two spellings, and each half needs its own. The
        # request takes DashScope's `W*H`; the recorded key takes `WxH`, or a
        # caller passing `512*512` and one passing `512x512` would bill the same
        # render under two aggregate keys. An absent size stays absent: sending
        # `size=""` rejects a no-size edit the base request would have accepted.
        recorded_size = folded_size.replace("*", "x")
        if folded_size:
            kwargs["size"] = folded_size.replace("x", "*")
                ...
                async with session.post(
                    ...
                    try:
                        ...
                        record_unusable_response(
                            ...
                            image_count=kwargs.get("n", 1),
                            # folded_size, not the request value: that one is
                            # the `W*H` wire form, and the aggregate key is
                            # `WxH`. Reading it back and converting would work
                            # but re-derives what is already in hand.
                            resolution=recorded_size,
                        )
                        raise
