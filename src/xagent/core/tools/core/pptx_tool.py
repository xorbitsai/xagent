...
// Dynamic font size estimation based on text length
const estimateFontSize = (text, baseSize, minSize = 18) => {{
  const length = text.length;
  if (length <= 15) return baseSize;
  if (length <= 30) return Math.max(baseSize * 0.8, minSize);
  if (length <= 50) return Math.max(baseSize * 0.65, minSize);
  return Math.max(baseSize * 0.5, minSize);
}};
...
"""
    ...
    def _build_timeline_slide_js(
        ...
        for i, ms in enumerate(milestones):
            ...
            js_lines.append(
                f"_slide_timeline.addText({json.dumps(ms_desc, ensure_ascii=False)}, {{ x: {x_pos}, y: 2.1, w: 2, h: 1.5, fontSize: ms{i}DescSize, color: {json.dumps(text_color, ensure_ascii=False)} }});"
            )
