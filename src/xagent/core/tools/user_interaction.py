...
class ToolInteractionSettlement:
    ...
    def __post_init__(self) -> None:
        ...
        if self.status == "succeeded":
            ...
            # Deferred import, required: ``..agent.result`` pulls in the
            # ``xagent.core.agent`` package, whose ``runtime`` imports this
            # module at module scope. Hoisting this to the top makes importing
            # xagent.core.tools.user_interaction first fail with "cannot
            # import name WAITING_FOR_USER_STATUS from partially initialized
            # module" (verified, not theoretical).
    ...
    def succeeded(cls, result: Any) -> ToolInteractionSettlement:
        """The tool call completed; ``result`` is its real terminal output."""
    ...
    def rejected(
        ...
        *,
        ...
        """The user denied the call; it definitely did not execute."""
    ...
    def failed(
        ...
        *,
        ...
        """The call definitely did not execute, and the reason is known.

        Use this only when the external effect provably did not happen. An
        ambiguous outcome (timeout, lost ack, unreadable response) is
        ``dispatch_unknown`` instead: only that status gets the duplicate-write
        guard and the final-answer fence, so mapping a maybe-executed write to
        ``failed`` silently drops that retry protection.
        """
    ...
    def dispatch_unknown(
        ...
        *,
        ...
        """The call may have executed; the external outcome is unknown.

        Counterpart to :meth:`failed`. The pattern treats this as a possibly
        completed write: it raises the settlement final-answer fence for the
        resumed turn and registers the record with the duplicate-write guard so
        a model retry of the same arguments is suppressed rather than executed.
        """
