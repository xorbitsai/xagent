...
Revision ID: 20260922_seed_rocketlane_mcp_app
Revises: 20260922_seed_fireflies_mcp_app
...
revision: str = "20260922_seed_rocketlane_mcp_app"
down_revision: Union[str, None] = "20260922_seed_fireflies_mcp_app"
...
# shape as the granola/notion/atlassian/miro seeds: a vendor-hosted remote MCP
# server reached over streamable_http, authenticated per user through OAuth 2.1
# + PKCE with Dynamic Client Registration, so there is no oauth_providers row
# and no secret in launch_config. Like the atlassian/miro seeds it carries the
...
# Rocketlane serves MCP under /mcp on a dedicated host, and its
# protected-resource metadata is published at the path-suffixed
# /.well-known/oauth-protected-resource/mcp (the host root returns 404) naming
...
def upgrade() -> None:
    ...
    if "launch_config" not in columns:
        raise RuntimeError(
            "Cannot seed builtin Rocketlane identity: "
    ...
    # No row claims app_id "rocketlane". A *different* app_id whose display
    # name (or, via typo/casing, its app_id) normalizes to the same identity is
    # a one-time cosmetic collision in the connector picker, not a
    # misidentification risk, so it does not warrant aborting the whole
    # `alembic upgrade head` run. Skip with a logged error instead.
    #
...
def downgrade() -> None:
    ...
    # oauth_providers row exists for Rocketlane (auth is per-user Dynamic
    # Client Registration, not a shared static client), and any
