...
5. When updating an agent, if you need to modify tools, skills, or knowledge bases, you MUST provide the FULL updated list in your tool call. If you do not include the existing ones, they will be removed! Connectors (`mcp`, `mcp:<server>`) are the exception: `tool_categories` only replaces the non-connector categories, the user manages connectors in the agent builder and they are kept automatically, so never pass them.
