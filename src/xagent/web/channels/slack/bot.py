...
class SlackBotInstance:
    ...
    async def _process_event(
        ...
        try:
            ...
            # its stale MCP config forever, and a Slack reply is one of the
            # ways a connect_apps pause gets answered. Gated on prior_status
            # (this task's status just before this claim, not its
            # now-RUNNING status) - an ordinary continuing message has no
            # reason to believe connector state changed, and invalidating a
            # warm cached agent's tools on every message forces a full
