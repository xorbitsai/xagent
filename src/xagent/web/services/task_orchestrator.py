...
def _accept_turn_no_commit(
    ...
    *,
    ...
    if not queued:
        # One read of the claimed row serves both projections below. The
        # workforce guard stays ahead of it on purpose: a rejected turn has to
        # raise TaskTurnError on its own terms, without depending on this row
        # still being readable (``.one()`` would raise NoResultFound instead).
        claimed_task = (
        ...
        # completion and resurrect the projection. The trigger projection is
        # deliberately not workforce-specific: any acceptance that takes a task
        # out of PAUSED / WAITING_FOR_USER has to take its trigger run out of the
        # parked state too, or a resumed run keeps reading as parked until the
