...
def test_dockerfile_vendor_path_matches_registry():
    """The vendored clone destination is pinned in two places: the registry's
    CHARTMOGUL_MCP_VENDOR_PATH constant (used in launch_config and covered by
    test_seed_row_matches_registry) and the Dockerfile's
    CHARTMOGUL_MCP_VENDOR_PATH build ARG default. An unsynchronized change to
    either would make ChartMogul's launch_config point at a directory that
    doesn't exist in the built image, so this closes the second side of that
    pairing the same way test_dockerfile_npx_cache_pin_matches_registry does
    for the chrome-devtools connector's npx version pin.
    """
    from xagent.web.builtin_mcp_registry import CHARTMOGUL_MCP_VENDOR_PATH

    assert CHARTMOGUL_MCP_VENDOR_PATH == EXPECTED_VENDOR_PATH
