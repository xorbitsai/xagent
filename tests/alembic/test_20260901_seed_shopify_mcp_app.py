"""Tests for the Shopify MCP connector seed migration."""
...
def _load_migration_module():
    migration_file = (
        ...
        / "src/xagent/migrations/versions/20260901_seed_shopify_mcp_app.py"
    )
    spec = importlib.util.spec_from_file_location(
        "seed_shopify_migration", migration_file
    )
...
def test_upgrade_inserts_shopify(tmp_path):
    ...
    with engine.begin() as connection:
        ...
        assert "xagent.web.tools.mcp.shopify" in str(row[2])
        for env_key in ("SHOPIFY_STORE_DOMAIN", "SHOPIFY_ACCESS_TOKEN"):
...
def test_seed_row_matches_registry():
    ...
    shopify row (the migration is a frozen copy; this catches drift)."""
    from xagent.web.builtin_mcp_registry import get_builtin_public_mcp_app_rows

    migration = _load_migration_module()
    registry_row = next(
        r for r in get_builtin_public_mcp_app_rows() if r["app_id"] == "shopify"
    )
    assert migration.ROW == registry_row


def test_downgrade_removes_shopify(tmp_path):
    engine = create_engine(f"sqlite:///{tmp_path / 'test.db'}")
    migration = _load_migration_module()
    with engine.begin() as connection:
        _create_table(connection)
        with patch.object(migration, "op", _operations(connection)):
            migration.upgrade()
            migration.downgrade()
        assert "shopify" not in _app_ids(connection)
