"""Tests for updating the GitHub connector's description."""
...
def _load_migration_module():
    migration_file = (
        ...
        / "src/xagent/migrations/versions/20260914_update_github_description.py"
    )
    spec = importlib.util.spec_from_file_location(
        "update_github_description_migration", migration_file
    )
...
def _create_table(
    ...
    connection.execute(
        text(
            ...
            f"VALUES ('github'{description_val})"
        ),
...
def _description(connection):
    return connection.execute(
        text("SELECT description FROM public_mcp_apps WHERE app_id='github'")
    ).scalar()
...
def test_upgrade_preserves_admin_customized_description(tmp_path):
    ...
    migration = _load_migration_module()
    with engine.begin() as connection:
        _create_table(connection, description="Our internal source control connector")
        with patch.object(migration, "op", _operations(connection)):
            migration.upgrade()
        assert _description(connection) == "Our internal source control connector"
    ...
    with engine.begin() as connection:
        ...
        with patch.object(migration, "op", _operations(connection)):
            ...
            connection.execute(
                ...
                {"d": "Our internal source control connector"},
            )
            migration.downgrade()
        assert _description(connection) == "Our internal source control connector"
