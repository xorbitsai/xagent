"""Tests for the SharePoint MCP connector seed migration."""
...
def _load_migration_module():
    migration_file = (
        ...
        / "src/xagent/migrations/versions/20260917_seed_sharepoint_mcp_app.py"
    )
    spec = importlib.util.spec_from_file_location(
        "seed_sharepoint_migration", migration_file
    )
...
def test_upgrade_inserts_sharepoint(tmp_path):
    ...
    with engine.begin() as connection:
        ...
        row = connection.execute(
            text(
                ...
                " WHERE app_id='sharepoint'"
            )
        ).first()
        assert row[0] == "oauth"
        assert row[1] == "microsoft"
        assert "xagent.web.tools.mcp.sharepoint" in str(row[2])
...
def test_upgrade_is_idempotent(tmp_path):
    ...
    with engine.begin() as connection:
        ...
        rows = connection.execute(
            text("SELECT COUNT(*) FROM public_mcp_apps WHERE app_id='sharepoint'")
        ).scalar()
        assert rows == 1
...
def test_seed_row_matches_registry(tmp_path):
    ...
    sharepoint row (the migration is a frozen copy; this catches drift)."""
    from xagent.web.builtin_mcp_registry import get_builtin_public_mcp_app_rows

    migration = _load_migration_module()
    registry_row = next(
        r for r in get_builtin_public_mcp_app_rows() if r["app_id"] == "sharepoint"
    )
    assert migration.ROW == registry_row


def test_downgrade_removes_sharepoint(tmp_path):
    engine = create_engine(f"sqlite:///{tmp_path / 'test.db'}")
    migration = _load_migration_module()
    with engine.begin() as connection:
        _create_table(connection)
...
def test_fresh_registry_seed_rejects_normalized_custom_server_collision(tmp_path):
    ...
    guard now covers sharepoint -- pin it the same way excel's is already
