...
async def test_unwrapped_providers_meter_and_surface_invalid_responses(
    ...
    """A provider must be correct with no retry policy above it.

    Every other test here drives a wrapped provider, so this combination would
    otherwise be untested: with no wrapper there is no retry policy, and
    ...
    (`model_service.get_image_models` now wraps too -- see
    test_model_service_wraps_image_models_with_the_retry_policy -- but the bare
    provider stays covered here rather than relying on the wrapper for
    correctness.)
    """
    from xagent.core.model.image import dashscope as ds_mod
    from xagent.core.model.image.base import InvalidImageResponseError
    ...
    # A bare provider, with no retry policy above it.
    model = ds_mod.DashScopeImageModel(
        model_name="wanx",
        api_key="k",
        abilities=["generate", "edit"],
