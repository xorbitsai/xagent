...
def test_the_access_slot_lets_the_boundary_error_through_its_wrapper(db_session):
    ...
    ``mcp.py``, and that path is covered by
    ``tests/web/api/test_mcp_team_connector_edit.py`` instead. The seam's own
    transient-outage error gets folded into ``ConnectorRuntimeError`` by the
    surrounding ``except Exception``; this one must not -- a permanent defect
    in the installing application's code is a different failure than an
    outage, and folding it in would also give this slot a different answer
    than every other checked slot for the same failure."""
