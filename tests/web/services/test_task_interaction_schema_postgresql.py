...
def test_origin_accepts_every_vocabulary_member(db_session, fixtures, origin) -> None:
    """Every member of the origin vocabulary is accepted -- see the SQLite
    half."""
    task_id, anchor_id = fixtures
    assert_accepted(
        db_session,
...
def test_terminated_status_and_terminated_at_are_paired_both_ways(
    ...
    """status='terminated' iff terminated_at IS NOT NULL -- see the SQLite half."""
    task_id, anchor_id = fixtures
    assert_rejected(
        db_session,
        make_row(
...
def test_answered_status_and_response_payload_are_paired_both_ways(
    ...
    """status='answered' iff response_payload IS NOT NULL -- see the SQLite
    half."""
    task_id, anchor_id = fixtures
    assert_rejected(
        db_session,
...
def test_answered_status_and_responded_at_are_paired_both_ways(
    ...
    """status='answered' iff responded_at IS NOT NULL -- see the SQLite
    half."""
    task_id, anchor_id = fixtures
    assert_rejected(
        db_session,
