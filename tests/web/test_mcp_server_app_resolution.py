...
class TestTheChangedCallers:
    ...
    def test_listing_keeps_expired_unrefreshable_account_label_hidden(self, db):
        """The new persisted-state field must not broaden the older account-label
        contract. A non-empty token remains ``connected`` in that projection,
        while an expired credential with no refresh token must not expose its
        stale email as a currently connected account."""
        from xagent.web.api.mcp import get_mcp_servers
        from xagent.web.models.user_oauth import UserOAuth
    ...
    def test_summaries_pick_the_newest_row_even_when_it_is_broken(self, db):
        ...
        assert (email, status) == (None, "needs_reconnect")
